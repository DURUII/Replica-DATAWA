...
class LSTMModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers=2, dropout=0.3):
        ...
        # Paper: "A Long Short-Term Memory model featuring a fully connected layer and an activation function."
        # Standard implementation: LSTM -> FC
        self.lstm = nn.LSTM(input_dim, hidden_dim, num_layers, batch_first=True, dropout=dropout)
        self.fc = nn.Linear(hidden_dim, output_dim)

    def forward(self, x):
        # x: (Batch, Seq_Len, Num_Nodes, Input_Dim)
        # Reshape to (Batch * Num_Nodes, Seq_Len, Input_Dim) to process each node's time series independent of neighbors (Baseline LSTM usually ignores graph structure)
...
## GraphWaveNet Model from https://github.com/nnzhan/Graph-WaveNet/blob/master/model.py
...
class GraphWaveNet(nn.Module):
    def __init__(self, num_nodes, input_dim, in_dim=None, out_dim=12, device='cpu',
        ...
        in_channels = input_dim if in_dim is None else in_dim
        self.core = gwnet(
            device=device,
            num_nodes=num_nodes,
            dropout=dropout,
    ...
    def forward(self, x):
        ...
        y = self.core(x)           # (B, out_dim, N, T)
        y_last = y[:, :, :, -1]    # (B, out_dim, N)
        y_last = y_last.permute(0, 2, 1)  # (B, N, out_dim)
        return y_last, None
